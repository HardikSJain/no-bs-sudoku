  ...
  group('QualityScore.compute', () {
    test('perfect solve at par time gives 100', () {
      final score = QualityScore.compute(
        timeSeconds: 900, hintDepthTotal: 0, mistakes: 0, undos: 0, difficulty: Difficulty.medium,
      );
      expect(score, 100.0);
    });
    ...
    test('under par time still caps at 100', () {
      final score = QualityScore.compute(
        timeSeconds: 300, hintDepthTotal: 0, mistakes: 0, undos: 0, difficulty: Difficulty.medium,
      );
      expect(score, 100.0);
    });
    ...
    test('at 3x par time gives 0 for time component', () {
      ...
      final score = QualityScore.compute(
        timeSeconds: 2700, hintDepthTotal: 0, mistakes: 0, undos: 0, difficulty: Difficulty.medium,
      );
      expect(score, 60.0);
    });
    ...
    test('beyond 3x par floors time at 0', () {
      final score = QualityScore.compute(
        timeSeconds: 5000, hintDepthTotal: 0, mistakes: 0, undos: 0, difficulty: Difficulty.medium,
      );
      expect(score, 60.0);
    });
    ...
    test('each mistake costs 10 points', () {
      ...
      final with2 = QualityScore.compute(
        timeSeconds: 900, hintDepthTotal: 0, mistakes: 2, undos: 0, difficulty: Difficulty.medium,
      );
    ...
    test('3 mistakes zeroes accuracy component', () {
      final score = QualityScore.compute(
        timeSeconds: 900, hintDepthTotal: 0, mistakes: 3, undos: 0, difficulty: Difficulty.medium,
      );
    ...
    test('each hint costs 7 points', () {
      ...
      final with1 = QualityScore.compute(
        timeSeconds: 900, hintDepthTotal: 6, mistakes: 0, undos: 0, difficulty: Difficulty.medium,
      );
    ...
    test('3 hints zeroes self-sufficiency component', () {
      final score = QualityScore.compute(
        timeSeconds: 900, hintDepthTotal: 18, mistakes: 0, undos: 0, difficulty: Difficulty.medium,
      );
    ...
    test('each undo costs 2 points', () {
      ...
      final with3 = QualityScore.compute(
        timeSeconds: 900, hintDepthTotal: 0, mistakes: 0, undos: 3, difficulty: Difficulty.medium,
      );
    ...
    test('5 undos zeroes confidence component', () {
      final score = QualityScore.compute(
        timeSeconds: 900, hintDepthTotal: 0, mistakes: 0, undos: 5, difficulty: Difficulty.medium,
      );
    ...
    test('worst case clamps to 0', () {
      final score = QualityScore.compute(
        timeSeconds: 99999, hintDepthTotal: 60, mistakes: 10, undos: 50, difficulty: Difficulty.medium,
      );
      expect(score, 0.0);
    });
    ...
    test('works for all difficulty levels', () {
      for (final diff in Difficulty.classic) {
        ...
        expect(score, 100.0, reason: '${diff.name} at par should be 100');
      }
    });
  });
